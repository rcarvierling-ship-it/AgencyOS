...
import { getAgencySettings, formatDate, formatDateTime } from '../../../lib/settings'
...
export default async function InquiriesPage() {
  const [inquiries, settings] = await Promise.all([getInquiries(), getAgencySettings()])
  return <AdminShell active="Inquiries" title="Project inquiries" subtitle="Website requests submitted through rcvagency.com.">
    <div className="stats"><div className="stat"><span>Inbound inquiries</span><strong>{inquiries.length}</strong><small>Real submissions</small></div><div className="stat"><span>Latest request</span><strong>{inquiries[0] ? formatDate(inquiries[0].createdAt, settings) : '—'}</strong><small>Most recent submission</small></div></div>
