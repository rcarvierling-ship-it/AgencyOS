...
export function AdminShell({ children, active = 'Overview', title, subtitle, action }: { children: React.ReactNode; active?: string; title?: string; subtitle?: string; action?: React.ReactNode }) {
  return <div className={`${styles.app} agencyAdmin`}>
    ...
    <main className={styles.main}>
      <header className={styles.header}><div><div className={styles.eyebrow}>AgencyOS · Command Center</div>{title && <h1>{title}</h1>}{subtitle && <p>{subtitle}</p>}</div><div className={styles.headerRight}><div className={styles.search}>⌕&nbsp; Search AgencyOS</div><Link className={styles.iconButton} href="/admin/settings">◐</Link><Link className={styles.profile} href="/admin/settings">RV</Link></div></header>
...
export function Stat({ label, value, delta, detail }: { label: string; value: string; delta?: string; detail?: string }) { return <div className="stat"><span>{label}</span><strong>{value}</strong>{delta && <small>↗ {delta} <i>{detail ?? 'vs. last month'}</i></small>}</div> }
export function Card({ title, eyebrow, children, href }: { title: string; eyebrow?: string; children: React.ReactNode; href?: string }) { return <section className="card">{(eyebrow || title) && <div className="cardHead"><div>{eyebrow && <span className={styles.eyebrow}>{eyebrow}</span>}<h2>{title}</h2></div>{href && <Link href={href}>View all →</Link>}</div>}{children}</section> }
...
export function EmptyState({ title, body, href, label }: { title: string; body: string; href?: string; label?: string }) { return <div className="empty"><div className="emptyIcon">✦</div><h3>{title}</h3><p>{body}</p>{href && <Link className="primary" href={href}>{label ?? 'Get started'} →</Link>}</div> }
