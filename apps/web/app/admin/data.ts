...
function sqlClient(): Database | null {
  const url = process.env.DATABASE_URL
  if (!url) return null
  return postgres(url, { prepare: false, max: 1 })
}

export async function withDb<T>(work: (sql: Database) => Promise<T>, fallback: T): Promise<T> {
  const sql = sqlClient()
...
export async function getBusinesses(limit = 100): Promise<BusinessRow[]> {
  return withDb<BusinessRow[]>(
    sql => sql<BusinessRow[]>`
      select id,name,slug,industry,website_url as "websiteUrl",city,state,status,
  ...
  return withDb(
    async sql => {
      const [business] = await sql<any[]>`
        select id,name,slug,legal_name as "legalName",industry,website_url as "websiteUrl",
               phone,email,address,city,state,postal_code as "postalCode",country,status,
               opportunity_score as "opportunityScore",notes,created_at as "createdAt",updated_at as "updatedAt"
        from businesses where slug=${slug} limit 1
      `
      if (!business) return null

      const [audit] = await sql<any[]>`
        select overall_score as "overallScore",design_score as "designScore",mobile_score as "mobileScore",
               performance_score as "performanceScore",seo_score as "seoScore",
               accessibility_score as "accessibilityScore",conversion_score as "conversionScore",
               findings,created_at as "createdAt"
        from website_audits where business_id=${business.id}
        order by created_at desc limit 1
      `
      const contacts = await sql<any[]>`
        select id,name,role,email,phone,source from business_contacts
        where business_id=${business.id} order by created_at desc
      `
      const activities = await sql<any[]>`
        select id,type,title,detail,created_at as "createdAt" from business_activities
        where business_id=${business.id} order by created_at desc limit 50
      `
      const [opportunity] = await sql<any[]>`
        select id,name,stage,value_cents as "valueCents",probability,lost_reason as "lostReason"
        from opportunities where business_id=${business.id}
        order by updated_at desc limit 1
      `
      return { business, audit, contacts, activities, opportunity }
    },
    null,
  )
}
  ...
  return withDb<any[]>(
    sql => sql<any[]>`
      select o.stage,count(*)::int as count,
             coalesce(json_agg(json_build_object('name',b.name,'slug',b.slug,'score',b.opportunity_score)
             order by b.updated_at desc) filter(where b.id is not null),'[]') as businesses
      from opportunities o join businesses b on b.id=o.business_id
      group by o.stage
      order by array_position(array['discovered','qualified','researching','demo_ready','contacted','interested','proposal','won','lost']::text[],o.stage)
    `,
    [],
  )
}
...
export async function getDemos() {
  return withDb<any[]>(
    sql => sql<any[]>`
      select d.id,d.slug,d.status,d.preview_url as "previewUrl",d.created_at as "createdAt",
             b.name,b.slug as "businessSlug",b.opportunity_score as score
...
export async function getOutreach() {
  return withDb<any[]>(
    sql => sql<any[]>`
      ...
      where a.type in ('outreach','email','follow_up','proposal')
         or a.title ilike '%email%' or a.title ilike '%outreach%'
...
export async function getWebsites() {
  return withDb<any[]>(
    sql => sql<any[]>`
      ...
      from production_websites w join businesses b on b.id=w.business_id
